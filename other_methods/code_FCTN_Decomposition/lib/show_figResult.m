function show_figResult(Re_msi,Omsi,minI,maxI,methodname,enList,band2show,bandnum)
%% Use to show the visual result

temp_tensor= Re_msi{1};
Nway = size(temp_tensor);
Lmethod = length(Re_msi);

if length(Nway)>3
    for ii = 1:Lmethod
    Re_msi{ii} = reshape(Re_msi{ii},Nway(1),Nway(2),[]);
    end
end
