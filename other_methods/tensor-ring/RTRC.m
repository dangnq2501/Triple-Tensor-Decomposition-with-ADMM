function [x,y,RC,run_time,errHist]=RTRC(tnsr,P,mu,flag, origin)
...
for n=1:L
    ...
    z{n}=zeros(J);
    order=[n:N 1:n-1];
    lambda=lambda+500/sqrt(SR*max([prod(J(order(1:L))),prod(J(order(L+1:N)))]));
...
for i=1:maxiter
    ...
    errHist(i) = norm(x(:)-origin(:))/norm(origin(:));
