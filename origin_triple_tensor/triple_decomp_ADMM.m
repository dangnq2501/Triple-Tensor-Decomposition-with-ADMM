function [A, B, C, O, errHist] = triple_decomp_ADMM_outlier(D, r, opts)
% INPUT:
%   D      : dữ liệu (n1×n2×n3)
%   r      : triple rank
...
%   O,E    : sparse components (và bản sao E)
%   errHist: lịch sử residual
...
for k = 1:maxIter
    %% 1) Update L (A,B,C) qua ALS trên T = D - O + Y_L/muL
    T = D - O + (1/muL)*Y_L;
    ...
    %% 2) Update O: giải min_O  (muL/2)||D-L-O + Y_L/muL||^2 + (muO/2)||O-E + Y_O/muO||^2
    % Đây chỉ là giải hệ tuyến tính khởi phương:
    R1 = D - L + (1/muL)*Y_L;
    R2 = E - (1/muO)*Y_O;
    O  = (muL*R1 + muO*R2) / (muL + muO);

    %% 3) Update E bằng soft-threshold trên O + Y_O/muO
    R3 = O + (1/muO)*Y_O;
    ...
    %% 6) Ghi nhận lỗi và kiểm tra điều kiện dừng
